...
function Pack.price(setName)
  ...
  return Pack.PRICE_BY_SET[setName] or Pack.PRICE
end
...
local function poolByRarity(setName)
  ...
  for rarity, pool in pairs(pools) do
    if #pool < 5 and setName ~= "PROMOTIONAL" then
      for _, card in ipairs(Cache.allCards()) do
        if card.kind ~= "energy" and card.rarity == rarity then
          pool[#pool + 1] = card.id
        end
      end
    elseif #pool < 1 and setName == "PROMOTIONAL" then
      for _, card in ipairs(Cache.allCards()) do
        if card.kind ~= "energy" and (card.rarity == "PROMOSTAR" or card.rarity == rarity) then
          pool[#pool + 1] = card.id
        end
      end
    end
